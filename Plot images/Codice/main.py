...
def main(p,csv_file,right_wr_cg,left_wr_cg,leg_cg,muscolar_cg,force_cg):
  #csv_file = file_name()
  lines = readfile(csv_file + '.csv')
  final_data_structure, mapping = construct_tensor_from_file(lines)
  pippo = np.stack(final_data_structure[p,:,:])
  max_x = max(pippo[:,0])
  min_x = min(pippo[:,0])
  max_y = max(pippo[:,1])
  min_y = min(pippo[:,1])
  max_z = max(pippo[:,2])
  min_z = min(pippo[:,2])
  valori = [max_x,max_y,max_z,min_x,min_y,min_z]
  ...
  couples_of_joints = [['Head', 'Neck'],#0                          #Fornisco tutte le coppie di giunti con cui fare i vettori
                       ...
                       ['Chest','Right Lower Shoulder'],#69
                       ['Chest','Left Lower Shoulder']]#70
  ...
  neck_chest_proj_on_fp, plot14 = vector_projection_plane(p,mapping,final_data_structure,'Neck','Chest','Chest',frontal_plane)

  chest_shoulder_right_proj_on_fp, plot15 = vector_projection_plane(p,mapping,final_data_structure,'Chest','Right Lower Shoulder','Chest',frontal_plane)
  ...
  angle_names_list = ['Right upper arm flection', 'Left upper arm flection', 'Right upper arm abduction', 'Left upper arm abduction',
                      ...
                      'Head bents backwards','Neck lateral extension angle', 'Neck torsion angle','Trunk flection angle',
  ...
  factors_list = ['MAIN', 'MAIN', 'PEJORATIVE', 'PEJORATIVE', 'PEJORATIVE', 'PEJORATIVE', 'MAIN', 'MAIN', 'MAIN', 'MAIN', 'MAIN','None',
                  'PEJORATIVE', 'PEJORATIVE', 'MAIN', 'PEJORATIVE', 'PEJORATIVE']
  couples_list = [[right_shoulder_arm_proj_on_sp, trunk_proj_on_sp], [left_shoulder_arm_proj_on_sp, trunk_proj_on_sp],
                  ...
                  ['Head','Right Palm',neck_chest_proj_on_fp,chest_shoulder_right_proj_on_fp],['Head','Left Palm',matrix_of_vectors[9],matrix_of_vectors[70]],[matrix_of_vectors[5], matrix_of_vectors[3]],[matrix_of_vectors[6],matrix_of_vectors[4]],
                  ...
                  [matrix_of_vectors[10], plane_z],[shoulder_vec_proj_on_fp, pelivs_vec_on_fp],[Plane2_vector,matrix_of_vectors[66]]]
  ...
  for idx in range(len(angle_names_list)):
    ...
    if angle_name == 'Right shoulder abduction':
      hand_position = z_positioning(p,mapping,final_data_structure,couple[0],couple[1])
      if hand_position:
        angle = 180 - angle_between_vectors(couple[2],couple[3])
      else:
        angle = 'No'
      ...
      elif angle_name == 'Trunk inclination angle' and bend_trunk_check:
        angle = angle_between_vectors(matrix_of_vectors[10], pelivs_vec_on_fp)

    data['angle'].append(angle)
    angles.append(angle)

  coefficients, global_rula_score, level_action = Rula_score(*angles, right_wr_cg, left_wr_cg,leg_cg, muscolar_cg, force_cg, bend_trunk_check)
  ...
  plot(matrix_of_vectors,frontal_plane,sagittal_plane,horizontal_plane,matrix_of_vectors[10],plane_y,*valori)
  ...
  return [global_rula_score, level_action]
